
<!-- toc:start -->

- [projects](projects/) — 2 min

**_Conceptual/**

  - [Lang](_Conceptual/Lang/) — 1.6 min
  - [Systems-Thinking](_Conceptual/Systems-Thinking/) — 0.3 min

**BigData/**

  - [APIs](BigData/APIs/) — 1.6 min
  - [Database-History](BigData/Database-History/) — 0.9 min
  - [Hadoop-Full](BigData/Hadoop-Full/) — 5 min
  - [Hadoop](BigData/Hadoop/) — 1.6 min
  - [MCP](BigData/MCP/) — 3.7 min
  - [Networking-1](BigData/Networking-1/) — 3.4 min
  - [Networking-2-Containers](BigData/Networking-2-Containers/) — 2.9 min
  - [Oreilly](BigData/Oreilly/) — 2.7 min
  - [Snowflake](BigData/Snowflake/) — 2.5 min
  - [Spark](BigData/Spark/) — 7.3 min

**BigData-AWS/**

  - [Analytics-OpenSearch](BigData-AWS/Analytics-OpenSearch/) — 0.4 min
  - [Analytics](BigData-AWS/Analytics/) — 0.2 min
  - [_AWS-MOC](BigData-AWS/AWS-MOC/) — 1 min
  - [Compute](BigData-AWS/Compute/) — 0.5 min
  - [Containers](BigData-AWS/Containers/) — 1 min
  - [DB-Aurora](BigData-AWS/DB-Aurora/) — 1.4 min
  - [DB-DynamoDB](BigData-AWS/DB-DynamoDB/) — 1.8 min
  - [DB-RDS](BigData-AWS/DB-RDS/) — 0.3 min
  - [DB](BigData-AWS/DB/) — 0.1 min
  - [MLAI](BigData-AWS/MLAI/) — 0.5 min

**LLM-RAG/**

  - [00-Building-Blocks](LLM-RAG/00-Building-Blocks/) — 2.3 min
  - [01-RAG-Intro](LLM-RAG/01-RAG-Intro/) — 2.2 min
  - [02-RAG-System-Design](LLM-RAG/02-RAG-System-Design/) — 0.8 min
  - [03-INDEXING](LLM-RAG/03-INDEXING/) — 2.8 min
  - [04-GENERATING](LLM-RAG/04-GENERATING/) — 2.6 min
  - [05-Evaluating](LLM-RAG/05-Evaluating/) — 3.5 min
  - [06-Adv-Systems](LLM-RAG/06-Adv-Systems/) — 3 min
  - [07-RAG-Ops](LLM-RAG/07-RAG-Ops/) — 2.2 min
  - [08-RAG-Variants](LLM-RAG/08-RAG-Variants/) — 3.3 min
  - [09-Current](LLM-RAG/09-Current/) — 0.8 min
  - [OpenTelemetry-Observability](LLM-RAG/OpenTelemetry-Observability/) — 0.8 min

**ML/**

  - [Meridian-Demo](ML/Meridian-Demo/) — 1.4 min
  - [ML-01](ML/ML-01/) — 4.2 min
  - [ML-03-Classification](ML/ML-03-Classification/) — 8.3 min
  - [ML-04-Regression](ML/ML-04-Regression/) — 1.2 min
  - [ML-07-Ensemble](ML/ML-07-Ensemble/) — 3.1 min
  - [ML-10-DL](ML/ML-10-DL/) — 5.7 min
  - [_ML-MOC](ML/ML-MOC/) — 0.3 min
  - [ML-TS](ML/ML-TS/) — 15.7 min

**Statistics/**

  - [Statistics-Taxonomy](Statistics/Statistics-Taxonomy/) — 4.1 min
  - [Stats-Glossary](Statistics/Stats-Glossary/) — 23.1 min
  - **Assets-Stats-Taxonomy/**
    - **1-Foundations/**
      - [Linear-Algebra](Statistics/Assets-Stats-Taxonomy/1-Foundations/Linear-Algebra/) — 4 min
      - [Probability-MOC](Statistics/Assets-Stats-Taxonomy/1-Foundations/Probability-MOC/) — 2.9 min
    - **2-Descriptive/**
      - [AutoEncoding](Statistics/Assets-Stats-Taxonomy/2-Descriptive/AutoEncoding/) — 0.9 min
      - [Clustering](Statistics/Assets-Stats-Taxonomy/2-Descriptive/Clustering/) — 2.5 min
      - [Dimensionality-Reduction](Statistics/Assets-Stats-Taxonomy/2-Descriptive/Dimensionality-Reduction/) — 1.2 min
      - [EDA](Statistics/Assets-Stats-Taxonomy/2-Descriptive/EDA/) — 0.3 min
      - [RelativePosition-EmpiricalRule-Chebyshev](Statistics/Assets-Stats-Taxonomy/2-Descriptive/RelativePosition-EmpiricalRule-Chebyshev/) — 1.7 min
      - [Similarity](Statistics/Assets-Stats-Taxonomy/2-Descriptive/Similarity/) — 1.4 min
      - [Topic-Modeling](Statistics/Assets-Stats-Taxonomy/2-Descriptive/Topic-Modeling/) — 3.4 min
      - [Univariate-Descriptive-Statistics](Statistics/Assets-Stats-Taxonomy/2-Descriptive/Univariate-Descriptive-Statistics/) — 2 min
    - **4-Extensions/**
      - [Anomaly-Detection](Statistics/Assets-Stats-Taxonomy/4-Extensions/Anomaly-Detection/) — 7.4 min
      - [Association-Rules](Statistics/Assets-Stats-Taxonomy/4-Extensions/Association-Rules/) — 1.1 min
      - [Bayes-MCMC](Statistics/Assets-Stats-Taxonomy/4-Extensions/Bayes-MCMC/) — 15.3 min
      - [Bayesian-Networks](Statistics/Assets-Stats-Taxonomy/4-Extensions/Bayesian-Networks/) — 1.6 min
      - **Stats-AB/**
        - [AB-Testing](Statistics/Assets-Stats-Taxonomy/4-Extensions/Stats-AB/AB-Testing/) — 1 min
        - [Google-AB-Course](Statistics/Assets-Stats-Taxonomy/4-Extensions/Stats-AB/Google-AB-Course/) — 2 min
  - **Cards/**
    - [Data-Types-Analysis-Choice](Statistics/Cards/Data-Types-Analysis-Choice/) — 2.2 min
    - [OLS-Assumptions-BLUE](Statistics/Cards/OLS-Assumptions-BLUE/) — 3.5 min
    - [Power](Statistics/Cards/Power/) — 1.5 min
    - [Quantile-plots](Statistics/Cards/Quantile-plots/) — 0.5 min
    - [R-functions-p-q-d-r](Statistics/Cards/R-functions-p-q-d-r/) — 0.6 min
    - [Regression](Statistics/Cards/Regression/) — 5.2 min
    - [Symbols](Statistics/Cards/Symbols/) — 4 min


<!-- toc:end -->
